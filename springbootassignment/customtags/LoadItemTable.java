...
public class LoadItemTable extends TagSupport {
	...
	public int doEndTag() throws JspException {
		...
		try {
			//if (session.getAttribute("invno") != null) {
				ArrayList<ItemDetailsMasterDto> data = (ArrayList<ItemDetailsMasterDto>) request.getAttribute("tableData");
	//			out.println("<form action=\"/SpringMvcShoppingPage/removeitems/remove\" method=\"post\">");
				...
				out.println("      <th scope=\"col\">Shop ID</th>");
				out.println("    </tr>");
				out.println("  </thead>");
				out.println("  <tbody>");
				for (int i = 0; i < data.size(); i++) {
					...
					out.println("      <td>" + data.get(i).getItemdescription() + "</td>");
					out.println("      <td>" + data.get(i).getItemunits() + "</td>");
					out.println("      <td>" + data.get(i).getItemprice() + "</td>");
					out.println("      <td>" + data.get(i).getShopid() + "</td>");
